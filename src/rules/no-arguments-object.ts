...
const messageIds = Object.freeze({
  forbiddenArgumentsObject: "`arguments` is not allowed."
});
...
const hasDeclaredArgumentsBinding = (scope: ScopeLike, name: string): boolean => {
...
export const noArgumentsObjectRule = createRule({
  ...
  create: (context: RuleContext) => ({
    Identifier: (node) => {
      ...
      if (hasDeclaredArgumentsBinding(scope, node.name)) {
        return;
      }
      context.report({
        node,
